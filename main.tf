...
resource "azurerm_mssql_firewall_rule" "allowmyClient" {
  ...
  start_ip_address = "135.180.240.255"
  end_ip_address   = "135.180.240.255"
}

resource "null_resource" "database_setup"{
  provisioner "local-exec" {
      command = "sqlcmd -S ${azurerm_mssql_server.sqlserver["${var.app_setup[0]}"].fully_qualified_domain_name} -U ${azurerm_mssql_server.sqlserver["${var.app_setup[0]}"].administrator_login} -P ${azurerm_mssql_server.sqlserver["${var.app_setup[0]}"].administrator_login_password} -d ${var.app_setup[1]} -i 02.sql"
